...
FFPSITES_URL=http://wolf-u.li/u/439
################################################################################
# Functions
function func_exit {
    echo $2
...
sites_updated=0
wget -nv $FFPSITES_URL -O $TMPFILE1
if [[ $? -ne 0 ]]; then
    dialog --title " Error " --backtitle " UW Site Loader " --msgbox "Could not downlod current definition file with wget command. Trying to use curl command." 10 40

    curl -L $FFPSITES_URL -o $TMPFILE1
    if [[ $? -ne 0 ]]; then
        dialog --title " Error " --backtitle " UW Site Loader " --msgbox "Could not download current definition file with curl command." 10 40
    else
        sites_updated=1
    fi
else
...
if [[ $sites_updated -eq 1 ]]; then
    dialog --title " Information " --backtitle " UW Site Loader " --infobox "Downloading definitions\nCompleted - Processing" 10 40
else
    dialog --title " Warning " --backtitle " UW Site Loader " --yesno "\nUnable to download the current definition file. Please update it manually from $FFPSITES_URL. and safe the content to $FFPSITES_FILE.new\n Continue without updating?" 0 0
    if [[ $? -ne 0 ]]; then
        clear
        echo "Please update it manually.\n It is located it at $FFPSITES_FILE.new"
        echo "Exiting"
        exit 1
    else
        cp $FFPSITES_FILE.new $TMPFILE1
    fi
fi
################################################################################
